...
st.title("Parasitologie-Quiz")

# 👉 Hole den gewählten Modus aus der Startseite
quiz_mode = st.session_state.get("quiz_mode")

if not quiz_mode:
    st.warning("Bitte wähle zuerst einen Quiz-Modus auf der Startseite.")
    st.stop()

# Funktion zum Starten oder Zurücksetzen des Quiz
def start_quiz(df, mode):
    for key in ["random_questions", "user_answers", "current_question_index"]:
        st.session_state.pop(key, None)

    num_questions = 10 if "Low" in mode else 20
